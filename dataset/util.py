...
def get_svhn2mnist(train, img_size):
    ...
    dataset = StylizedDataset(root="./data/svhn2mnist", classes=[str(i) for i in range(10)], transform=transform_mnist_train)
...
def get_mnist2mnist_m(train, img_size):
    ...
    dataset = StylizedDataset(root="./data/mnist2mnist_m", classes=[str(i) for i in range(10)], transform=transform_mnist_train)
...
#             transforms.Lambda(lambda x: x.repeat(3, 1, 1)),
    ...
    dataset = torchvision.datasets.FashionMNIST(root='./data', train=train, download=True, transform=transform_mnist_train)
    dataset_len = len(dataset)
    if train:
        train_set, val_set = torch.utils.data.random_split(dataset, [50000, dataset_len-50000])
        return train_set, None
...
def get_emnist_m_letters(train, img_size):
    ...
    dataset = EMNISTMLetters(root='./data', download=True, transform=transform_mnist_train)
...
def get_fmnist_m(train, img_size):
    ...
    dataset = FashionMNISTM(root='./data', download=True, transform=transform_mnist_train)
...
#     ])
    ...
    dataset_train = OfficeHome(root="./data/officehome", task=domain_type, download=True, transform=transform_train)
    dataset_test = OfficeHome(root="./data/officehome", task=domain_type, download=True, transform=transform_test)
